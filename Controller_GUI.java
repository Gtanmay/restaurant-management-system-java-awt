...
public class Controller_GUI
{
    ...
    private int         orderIdCounter = 0;     //order counter
    ...
    public void setTodaysOrderCnt(int count)
    {
        this.todaysOrderCnt=count;
    }
            ...
            if( rStaff.getClass().getName().equalsIgnoreCase(searchClassName))
            {
                if(rStaff.getPassword().equals(inputPassword))
                {
                    if(rStaff.getWorkState() == 0)  //Not clocked in yet
                    {
                        ...
                        rStaff.clockIn(currDate);
                        cDatabase.markAttendanceInDB(inputID, currDate,"CLOCK_IN");
                    }
                    if(isManager)
                    {
     ...
     * Staff management
     **********************************************************/
    public boolean addNewStaff(String newID, String newPassword, String newFirstName, String newLastName, boolean isManager)
    {
    ...
    public boolean updateStaff(String id, String newPassword, String newFirstName, String newLastName)
    {
        try
        {
    ...
    public boolean clockOut(String staffID)
    {
        ...
        switch(state)
        {
            case Staff.WORKSTATE_ACTIVE:
                // rStaff.clockOut(new Date(System.currentTimeMillis()));
                Date currDate= new Date(System.currentTimeMillis());
                rStaff.clockOut(currDate);
                cDatabase.markAttendanceInDB(staffID, currDate, "CLOCK_OUT");
                result = true;
                break;
    ...
    public boolean addNewOrderItem(int orderID, int addItemID, byte addItemQuantity)
    {
        ...
        if( !currentUserID.trim().equalsIgnoreCase(rOrder.getStaffID().trim()) && userType != 2)
        {
        ...
        if(rNewItem == null)
        {
         ...
         //////////ADD!!!(database)/////////////
         cDatabase.addOrderItem(orderID, rNewItem, addItemQuantity, true);
         return true;
    }
    ...
    {
        Order rOrder = cDatabase.findOrderByID(orderID);
        // if( currentUserID != rOrder.getStaffID() && userType != 2)
        if( !currentUserID.trim().equalsIgnoreCase(rOrder.getStaffID().trim()) && userType != 2)
    ...
    public  ArrayList<String>  createStaffList()
    {
        ...
        while (it.hasNext()) {
            ...
            String output = String.format("Staff ID:%-25s  Name:%-25s",
                                            re.getID(), fullName);
    ...
    //private void createOrderItemlList( int orderID, JList list)
    {
        ...
        while (it.hasNext()) {
            re = it.next();
            output = String.format("%-4d %-20s   %5d       %5.2f",
    ...
    public String createPaymentList()
    {
        ...
        while (it.hasNext())
        {
            ...
            if(re.getWorkState() == Staff.WORKSTATE_FINISH)
            {
                ...
                output += String.format("ID:%4d  Name:%-20s  Work time:%5.2f Pay:%5.2f\n",
            ...
            {
                output += String.format("ID:%4d  Name:%-20s  * On work *\n",
                                            Integer.parseInt(re.getID()),re.getFullName());
                staffNum++;
            }
        }
        output += "-------------------------------------------------------\n";
